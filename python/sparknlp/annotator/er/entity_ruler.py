...
class EntityRulerApproach(AnnotatorApproach, HasStorage):
    ...
    def setPatternsResource(
        self, path, read_as=ReadAs.TEXT, options={"format": "JSON"}
    ):
...
class EntityRulerModel(AnnotatorModel, HasStorageModel):
    ...
    def __init__(
        self,
        classname="com.johnsnowlabs.nlp.annotators.er.EntityRulerModel",
        java_model=None,
    ):
    ...
    def pretrained(name, lang="en", remote_loc=None):
        ...
        return ResourceDownloader.downloadModel(
            EntityRulerModel, name, lang, remote_loc
        )

    @staticmethod
    def loadStorage(path, spark, storage_ref):
        HasStorageModel.loadStorages(
            path, spark, storage_ref, EntityRulerModel.databases
        )
