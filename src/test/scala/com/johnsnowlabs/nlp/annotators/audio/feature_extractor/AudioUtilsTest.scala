...
class AudioUtilsTest extends AnyFlatSpec {
  ...
  it should "create mel spectrogram" taggedAs FastTest in {
    // TODO: Use other reference value for test
    val waveform: DenseVector[Double] =
      csvread(new File("src/test/resources/audio/csv/librispeech_asr_0_normed.csv")).toDenseVector
    ...
    val window: DenseVector[Double] = getHanningWindow()

    val melFilterBank: DenseMatrix[Double] = AudioUtils.melFilterBank(
    ...
    val spectogram: DenseMatrix[Double] = AudioUtils.calculateSpectrogram(
      waveform = waveform,
      window = window,
      frameLength = n_fft,
    ...
    val expectedLogMelSpec =
      csvread(new File("src/test/resources/audio/csv/librispeech_asr_0_logMelSpec.csv"))

    spectogram.valuesIterator.zip(expectedLogMelSpec.valuesIterator).foreach {
      case (specVal, expectedVal) =>
        assert(specVal === expectedVal)
    }
  }
